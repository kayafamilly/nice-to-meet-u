...
export async function POST(request: NextRequest, { params }: { params: Promise<{ sessionId: string }> }) {
  try {
    ...
    assertCsrf(request);
    assertRateLimit(request, "session-host-cancel", 10, 10 * 60 * 1000);
    const session = await getServerSession();
    if (!session) return unauthorized();
    const { sessionId } = await params;
    return noStoreJson(await callBusinessRoute(`/api/ntmy/sessions/${sessionId}/cancel`, { method: "POST" }, session));
  } catch (error) {
