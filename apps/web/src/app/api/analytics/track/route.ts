...
export async function POST(request: NextRequest) {
  try {
    ...
    const visitor = request.cookies.get(COOKIE)?.value ?? randomBytes(24).toString("base64url");
    ...
    if (!request.cookies.has(COOKIE)) response.cookies.set(COOKIE, visitor, { httpOnly: true, secure: new URL(getServerEnv().NEXT_PUBLIC_APP_URL).protocol === "https:", sameSite: "lax", maxAge: 90 * 86400, path: "/" });
    return response;
